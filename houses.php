<?php 	session_start();
	...
	if(!isset($_GET['idh'])) header('location: index.php');
	$sectorID = $_GET['idh'];
  //Include Ajax
  include('ajax.php');
?>
...
<h2>Houses in <?php echo $_GET['sector'];?></h2>
</div>
</section>
<!--  Page top end -->
<center>
	...
	<?php endforeach; endif; ?>
	    ...
	    <select id="display" class="col-md-3">
	        <option value="<?php echo $sectorID ?>"><?php echo  $_GET['sector']; ?></option>
	    </select>
...
<div class="row">
	<?php
	if(!isset($_GET['range'])): $query= $db->getRows('houses', array('orderd by' => 'id asc', 'where'=>array('status'=>0, 'sector_id'=>$sectorID)));
	elseif(isset($_GET['range'])):
		if($_GET['range']==1):
			$query= $db->getRows('houses', array('order_by' => 'id asc', 'start'=>0, 'limit'=>12,'where'=>array('status'=>0, 'sector_id'=>$sectorID)));
		elseif($_GET['range']==2):
			$query= $db->getRows('houses', array('order_by' => 'id asc', 'start'=>13, 'limit'=>12,'where'=>array('status'=>0, 'sector_id'=>$sectorID)));
		elseif($_GET['range']==3):
			$query= $db->getRows('houses', array('order_by' => 'id asc', 'start'=>25, 'limit'=>12,'where'=>array('status'=>0, 'sector_id'=>$sectorID)));
		elseif($_GET['range']==4):
			$query= $db->getRows('houses', array('order_by' => 'id asc', 'start'=>37, 'limit'=>12,'where'=>array('status'=>0, 'sector_id'=>$sectorID)));
		endif;
	endif;
	$count=0;
					...
					<?php endif; ?>
						...
						<div class="feature-text">
							...
							<?php endforeach; endif; ?>
								...
								<a href="#" class="room-price">$<?php echo $show['price']; ?></a>
						</div>
					</a>
				</div>
</div>
<?php endforeach;
else: echo '<div class="col-lg-12 col-md-12"><center><div class="section-title text-center">No Result found.
</div></center></div>';
 endif; ?>
</div>
